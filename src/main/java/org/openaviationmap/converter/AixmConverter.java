...
public final class AixmConverter {
     ...
     */
    public static final String AIXM_NS_URI = "http://www.aixm.aero/schema/5.1";
     ...
     */
    public static final String AIXM_MESSAGE_NS_URI = "http://www.aixm.aero/schema/5.1/message";
     ...
     * related objects.
     */
    private static aero.aixm.schema._5.ObjectFactory aixmFactory = new aero.aixm.schema._5.ObjectFactory();

    /**
     * An AIXM message object factory, generated by JAXB, used to create AIXM
     * related objects.
     */
    private static aero.aixm.schema._5_1.message.ObjectFactory aixmMessageFactory = new aero.aixm.schema._5_1.message.ObjectFactory();
     ...
     */
    public static JAXBElement<AIXMBasicMessageType> convertToAixm(
            List<Airspace> airspaces, List<Navaid> navaids,
            List<Aerodrome> aerodromes, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction) {

        AIXMBasicMessageType message = aixmMessageFactory
                .createAIXMBasicMessageType();

        airspacesToAixm(airspaces, validStart, validEnd, interpretation,
                sequence, correction, message.getHasMember());

        navaidsToAixm(navaids, validStart, validEnd, interpretation, sequence,
                correction, message.getHasMember());

        aerodromesToAixm(aerodromes, validStart, validEnd, interpretation,
                sequence, correction, message.getHasMember());

        JAXBElement<AIXMBasicMessageType> m = aixmMessageFactory
                .createAIXMBasicMessage(message);
     ...
     *        this list
     */
    static void airspaceToAixm(Airspace airspace, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        AirspaceTimeSlicePropertyType sliceProp = aixmFactory
                .createAirspaceTimeSlicePropertyType();
        AirspaceTimeSliceType slice = aixmFactory.createAirspaceTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        ...
        // set the designator
        CodeAirspaceDesignatorType designator = aixmFactory
                .createCodeAirspaceDesignatorType();
        ...
        LinguisticNotePropertyType enNoteT = aixmFactory
                .createLinguisticNotePropertyType();
        ...
        AirspaceVolumeType airspaceVolume = aixmFactory
                .createAirspaceVolumeType();
        ...
        AirspaceVolumePropertyType airspaceVolumeProp = aixmFactory
                .createAirspaceVolumePropertyType();
        airspaceVolumeProp.setAirspaceVolume(airspaceVolume);

        AirspaceGeometryComponentType geometry = aixmFactory
                .createAirspaceGeometryComponentType();
        geometry.setTheAirspaceVolume(airspaceVolumeProp);

        AirspaceGeometryComponentPropertyType geometryProp = aixmFactory
                .createAirspaceGeometryComponentPropertyType();
        ...
        BasicMessageMemberAIXMPropertyType p = aixmMessageFactory
     ...
     */
    private static TimePrimitivePropertyType convertTimePeriod(
        ...
        TimePrimitivePropertyType tp = gmlFactory
     ...
     */
    private static TimePositionType calendarToTimePosition(
            GregorianCalendar time) {
        dateFormatter.setTimeZone(TimeZone.getTimeZone("UTC"));
        TimePositionType timePos = gmlFactory.createTimePositionType();
        if (time != null) {
            timePos.getValue().add(dateFormatter.format(time.getTime()));
        } else {
            timePos.setIndeterminatePosition(TimeIndeterminateValueType.UNKNOWN);
        }
     ...
     */
    private static void airspaceControlActivationToAixm(Airspace airspace,
            AirspaceTimeSliceType slice) {
        // set the control type
        CodeMilitaryOperationsType controlType = aixmFactory
                .createCodeMilitaryOperationsType();
        ...
        if ("H24".equals(airspace.getActiveTime())) {
            ...
            CodeTimeReferenceType tRef = aixmFactory
                    .createCodeTimeReferenceType();
            tRef.setValue("UTC");
            ...
            TimesheetPropertyType tsp = aixmFactory
                    .createTimesheetPropertyType();
            tsp.setTimesheet(ts);

            AirspaceActivationType act = aixmFactory
                    .createAirspaceActivationType();
            act.getTimeInterval().add(tsp);

            AirspaceActivationPropertyType actProp = aixmFactory
                    .createAirspaceActivationPropertyType();
            actProp.setAirspaceActivation(act);
            slice.getActivation().add(actProp);
        }

        CodeAirspaceActivityType actv = aixmFactory
                .createCodeAirspaceActivityType();
        ...
        if (airspace.getRemarks() != null && !airspace.getRemarks().isEmpty()) {
            ...
            if (remarks.contains("nuclear")) {
                ...
                actv.setValue("BIRD");
            } else if (remarks.contains("glide") || remarks.contains("gliding")) {
    ...
    private static void airspaceLimitsToAixm(Airspace airspace,
        ...
        // set the lower limit
        airspaceVolume
                .setLowerLimit(convertElevation(airspace.getLowerLimit()));

        CodeVerticalReferenceType lowerLimitRef = aixmFactory
                .createCodeVerticalReferenceType();
        lowerLimitRef.setValue(airspace.getLowerLimit().getReference()
                .toString());
        airspaceVolume.setLowerLimitReference(lowerLimitRef);

        // set the upper limit
        airspaceVolume
                .setUpperLimit(convertElevation(airspace.getUpperLimit()));

        CodeVerticalReferenceType upperLimitRef = aixmFactory
                .createCodeVerticalReferenceType();
        upperLimitRef.setValue(airspace.getUpperLimit().getReference()
                .toString());
    ...
    private static void boundaryToAixm(Boundary boundary,
        ...
        CurveSegmentArrayPropertyType curveSegment = gmlFactory
        ...
        AbstractRingPropertyType abstractRingProp = gmlFactory
        ...
        SurfacePatchArrayPropertyType patchArray = gmlFactory
        ...
        SurfacePropertyType surfacePropType = aixmFactory
     ...
     */
    private static void addCircleToCurveSegment(Circle circle,
        ...
        CircleByCenterPointType ccp = gmlFactory
     ...
     */
    private static void addRingToCurveSegment(Ring ring,
            CurveSegmentArrayPropertyType curveSegment) {
        DirectPositionListType directPosList = gmlFactory
        ...
        GeodesicStringType geoString = gmlFactory.createGeodesicStringType();
     ...
     *        this list
     */
    public static void airspacesToAixm(List<Airspace> airspaces,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {

        for (Airspace ap : airspaces) {
            airspaceToAixm(ap, validStart, validEnd, interpretation, sequence,
                    correction, propList);
        }
    }
     ...
     */
    static void navaidToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {

        List<AbstractNavaidEquipmentType> equipmentList = new Vector<AbstractNavaidEquipmentType>();
        ...
        case VOR:
        case VOT:
            vorToAixm(navaid, validStart, validEnd, interpretation, sequence,
                    correction, propList, equipmentList);
            navaidType = "VOR";
            break;

        case VORTAC:
            vortacToAixm(navaid, validStart, validEnd, interpretation,
                    sequence, correction, propList, equipmentList);
            navaidType = "VOR_TAC";
            break;

        case TACAN:
            tacanToAixm(navaid, validStart, validEnd, interpretation, sequence,
                    correction, propList, equipmentList);
            navaidType = "TACAN";
            break;

        case DME:
            dmeToAixm(navaid, validStart, validEnd, interpretation, sequence,
                    correction, propList, equipmentList);
            navaidType = "DME";
            break;

        case NDB:
            ndbToAixm(navaid, validStart, validEnd, interpretation, sequence,
                    correction, propList, equipmentList);
            navaidType = "NDB";
            break;

        case NDBDME:
            ndbdmeToAixm(navaid, validStart, validEnd, interpretation,
                    sequence, correction, propList, equipmentList);
            navaidType = "NDB_DME";
            break;
        ...
        if (navaidType != null) {
            navaidForNavaidEquipment(navaid, navaidType, equipmentList,
                    validStart, validEnd, interpretation, sequence, correction,
                    propList);
        }
     ...
     *         this list
     */
    static void vorToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
        ...
        ElevatedPointType ep = convertPointElevation(navaid,
                navaid.getElevation());
        ElevatedPointPropertyType epp = aixmFactory
                .createElevatedPointPropertyType();
        ...
        if (navaid.getVariation() != null) {
            ...
            var.setValue(doubleToBigDecimal(navaid.getVariation()
                    .getVariation()));
        ...
        // the designator
        CodeNavaidDesignatorType des = aixmFactory
                .createCodeNavaidDesignatorType();
        ...
        VORTimeSlicePropertyType sliceProp = aixmFactory
                .createVORTimeSlicePropertyType();
        ...
        BasicMessageMemberAIXMPropertyType prop = aixmMessageFactory
                .createBasicMessageMemberAIXMPropertyType();
    ...
    /**
     * Convert a TACAN into an AIXM navaid.
     *
     * @param navaid the navigation aid to convert
     * @param validStart the beginning of the validity period for the
     *        aerospace features. if null, this is unknown.
     ...
     *         this list
     */
    //TODO Review the TACAN generation. At the very least, this does not set the frequency
    static void tacanToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        // the type
        //TODO
        TACANType type = aixmFactory.createTACANType();

        // **********
        // Create the TACAN AIXMFeature node
        ...
        // The TACAN channel
        CodeTACANChannelType tacanChannel = aixmFactory
                .createCodeTACANChannelType();
        tacanChannel.setValue(navaid.getDmeChannel());

        // package the things together
        TACANTimeSliceType slice = aixmFactory.createTACANTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        slice.setLocation(epp);
        slice.setMagneticVariation(var);
        ...
        slice.setDesignator(des);
        slice.setAixmName(name);
        slice.setChannel(tacanChannel);

        TACANType tacan = aixmFactory.createTACANType();

        TACANTimeSlicePropertyType sliceProp = aixmFactory
                .createTACANTimeSlicePropertyType();
        sliceProp.setTACANTimeSlice(slice);

        tacan.getTimeSlice().add(sliceProp);
        tacan.setIdentifier(id);
        tacan.setId("uuid." + id.getValue());

        BasicMessageMemberAIXMPropertyType prop = aixmMessageFactory
                .createBasicMessageMemberAIXMPropertyType();
        prop.setAbstractAIXMFeature(aixmFactory.createTACAN(tacan));

        propList.add(prop);

        equipmentList.add(tacan);
    }
     ...
     *         this list
     */
    static void dmeToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        DMETimeSlicePropertyType sliceProp = aixmFactory
                .createDMETimeSlicePropertyType();
     ...
     *         this list
     */
    static void ndbToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        NDBTimeSlicePropertyType sliceProp = aixmFactory
                .createNDBTimeSlicePropertyType();
    ...
    /**
     * Convert a NDB/DME into the two corresponding AIXM navaid.
     *
     * @param navaid the navigation aid to convert
     * @param validStart the beginning of the validity period for the
     *        aerospace features. if null, this is unknown.
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    //TODO I believe that ideally, these should be combined into a composit navaid
    static void ndbdmeToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
    ...
    /**
     * Convert a marker beacon into an AIXM navaid.
     *
     * @param navaid the navigation aid to convert
     * @param validStart the beginning of the validity period for the
     *        aerospace features. if null, this is unknown.
     ...
     *         this list
     */
    static void markerToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        String auralStr = navaid.getIdent().replace("-", "")
                .replace("Dashes", "-").replace("Dots", ".");
        auralCode.setValue(auralStr);

        // package the things together
        MarkerBeaconTimeSliceType slice = aixmFactory
                .createMarkerBeaconTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        slice.setLocation(epp);
        slice.setMagneticVariation(var);
        ...
        MarkerBeaconTimeSlicePropertyType sliceProp = aixmFactory
                .createMarkerBeaconTimeSlicePropertyType();
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    static void designatedToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        // the designator
        CodeDesignatedPointDesignatorType des = aixmFactory
                .createCodeDesignatedPointDesignatorType();
        des.setValue(navaid.getIdent());
        ...
        // the type
        CodeDesignatedPointType type = aixmFactory
                .createCodeDesignatedPointType();
        type.setValue("ICAO");
        ...
        // package the things together
        DesignatedPointTimeSliceType slice = aixmFactory
                .createDesignatedPointTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        ...
        DesignatedPointTimeSlicePropertyType sliceProp = aixmFactory
                .createDesignatedPointTimeSlicePropertyType();
     ...
     *         this list
     */
    static void localizerToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        // get the decliation
        ValMagneticVariationType declination = aixmFactory
                .createValMagneticVariationType();
        declination.setValue(doubleToBigDecimal(navaid.getDeclination()));

        // get the magnetic variation
        ValMagneticVariationType variation = aixmFactory
                .createValMagneticVariationType();
        DateYearType variationDate = aixmFactory.createDateYearType();
        if (navaid.getVariation() != null) {
            variation.setValue(doubleToBigDecimal(navaid.getVariation()
                    .getVariation()));
            variationDate.setValue(Integer.toString(navaid.getVariation()
                    .getYear()));
        }

        // package the things together
        LocalizerTimeSliceType slice = aixmFactory
                .createLocalizerTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        slice.setLocation(epp);
        slice.setDesignator(des);
        ...
        LocalizerTimeSlicePropertyType sliceProp = aixmFactory
                .createLocalizerTimeSlicePropertyType();
     ...
     *         this list
     */
    static void glidepathToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
        ...
        ValDistanceVerticalType elevation = convertElevation(navaid
                .getElevation());
        ...
        // package the things together
        GlidepathTimeSliceType slice = aixmFactory
                .createGlidepathTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        slice.setLocation(epp);
        ...
        GlidepathTimeSlicePropertyType sliceProp = aixmFactory
                .createGlidepathTimeSlicePropertyType();
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    static void aerodromeToAixm(Aerodrome aerodrome,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        aerodromeNavaidsToAixm(aerodrome, id.getValue(), validStart, validEnd,
                interpretation, sequence, correction, propList);

        airspacesToAixm(aerodrome.getAirspaces(), validStart, validEnd,
                interpretation, sequence, correction, propList);

        aerodromeFrequenciesToAixm(aerodrome, id.getValue(), validStart,
                validEnd, interpretation, sequence, correction, propList);

        aerodromeRunwaysToAixm(aerodrome, id.getValue(), validStart, validEnd,
                interpretation, sequence, correction, propList);
        ...
        CodeAirportHeliportDesignatorType designator = aixmFactory
                .createCodeAirportHeliportDesignatorType();
        ...
        CodeAirportHeliportType type = aixmFactory
                .createCodeAirportHeliportType();
        type.setValue("AD");

        CodeYesNoType certifiedIcao = aixmFactory.createCodeYesNoType();
        certifiedIcao.setValue("yes");

        ValDistanceVerticalType elevation = convertElevation(aerodrome
                .getElevation());

        ElevatedPointType arp = convertPointElevation(aerodrome.getArp(),
                aerodrome.getElevation());
        ElevatedPointPropertyType arpProp = aixmFactory
                .createElevatedPointPropertyType();
        arpProp.setElevatedPoint(arp);

        // get the remarks as a note
        CodeNotePurposeType remarkPurpose = aixmFactory
                .createCodeNotePurposeType();
        ...
        LinguisticNotePropertyType lnp = aixmFactory
                .createLinguisticNotePropertyType();
        ...
        // package the things together
        AirportHeliportTimeSliceType slice = aixmFactory
                .createAirportHeliportTimeSliceType();
        initTimeSlice(slice, validStart, validEnd, interpretation, sequence,
                correction);
        slice.setAixmName(name);
        slice.setDesignator(designator);
        ...
        AirportHeliportTimeSlicePropertyType sliceProp = aixmFactory
                .createAirportHeliportTimeSlicePropertyType();
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void aerodromeFrequenciesToAixm(Aerodrome aerodrome,
            String aerodromeId, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {
        // add the frequencies, if aplicable
        if (aerodrome.getAfis() != null) {
            commFrequencyToAixm(aerodrome.getAfis(), aerodromeId,
                    aerodrome.getName() + " Info", "AFIS", validStart,
                    validEnd, interpretation, sequence, correction, propList);
        }

        if (aerodrome.getTower() != null) {
            commFrequencyToAixm(aerodrome.getTower(), aerodromeId,
                    aerodrome.getName() + " Tower", "TWR", validStart,
                    validEnd, interpretation, sequence, correction, propList);
        }

        if (aerodrome.getAtis() != null) {
            commFrequencyToAixm(aerodrome.getAtis(), aerodromeId,
                    aerodrome.getName() + " ATIS", "ATIS", validStart,
                    validEnd, interpretation, sequence, correction, propList);
        }

        if (aerodrome.getApproach() != null) {
            commFrequencyToAixm(aerodrome.getApproach(), aerodromeId,
                    aerodrome.getName() + " Approach", "APP", validStart,
                    validEnd, interpretation, sequence, correction, propList);
        }

        if (aerodrome.getApron() != null) {
            commFrequencyToAixm(aerodrome.getApron(), aerodromeId,
                    aerodrome.getName() + " Ground", "GND", validStart,
                    validEnd, interpretation, sequence, correction, propList);
        }
    }
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void aerodromeNavaidsToAixm(Aerodrome aerodrome,
            String aerodromeId, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {

        // process the navaids associated with this aerodrome
        List<BasicMessageMemberAIXMPropertyType> navaids = new Vector<BasicMessageMemberAIXMPropertyType>();

        navaidsToAixm(aerodrome.getNavaids(), validStart, validEnd,
                interpretation, sequence, correction, navaids);
        ...
        for (BasicMessageMemberAIXMPropertyType prop : navaids) {
            ...
            if (prop.getAbstractAIXMFeature().getDeclaredType()
                ...
                AirportHeliportPropertyType ahp = aixmFactory
                        .createAirportHeliportPropertyType();
                ahp.setHref("#uuid." + aerodromeId);

                NavaidType n = (NavaidType) prop.getAbstractAIXMFeature()
                        .getValue();
            ...
            if (prop.getAbstractAIXMFeature().getDeclaredType()
                ...
                DesignatedPointType d = (DesignatedPointType) prop
                        .getAbstractAIXMFeature().getValue();

                for (DesignatedPointTimeSlicePropertyType dSlice : d
                        .getTimeSlice()) {
     ...
     *        this list
     */
    private static void commFrequencyToAixm(Frequency frequency,
            String aerodromeId, String frequencyName, String frequencyType,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {

        RadioCommunicationChannelTimeSliceType commSlice = aixmFactory
                .createRadioCommunicationChannelTimeSliceType();
        ...
        RadioCommunicationChannelTimeSlicePropertyType commSliceProp = aixmFactory
                .createRadioCommunicationChannelTimeSlicePropertyType();

        commSliceProp.setRadioCommunicationChannelTimeSlice(commSlice);

        CodeWithAuthorityType commId = gmlFactory.createCodeWithAuthorityType();
        commId.setCodeSpace("urn:uuid:");
        commId.setValue(UUID.randomUUID().toString());

        RadioCommunicationChannelType comm = aixmFactory
                .createRadioCommunicationChannelType();
        ...
        BasicMessageMemberAIXMPropertyType prop = aixmMessageFactory
                .createBasicMessageMemberAIXMPropertyType();
        prop.setAbstractAIXMFeature(aixmFactory
                .createRadioCommunicationChannel(comm));
        ...
        CallsignDetailType callsign = aixmFactory.createCallsignDetailType();
        callsign.setCallSign(callsignName);

        CallsignDetailPropertyType callsignProp = aixmFactory
                .createCallsignDetailPropertyType();
        callsignProp.setCallsignDetail(callsign);

        RadioCommunicationChannelPropertyType commProp = aixmFactory
                .createRadioCommunicationChannelPropertyType();
        ...
        AirTrafficControlServiceTimeSliceType atcsSlice = aixmFactory
                .createAirTrafficControlServiceTimeSliceType();
        ...
        AirTrafficControlServiceTimeSlicePropertyType atcsSliceProp = aixmFactory
                .createAirTrafficControlServiceTimeSlicePropertyType();
        atcsSliceProp.setAirTrafficControlServiceTimeSlice(atcsSlice);

        AirTrafficControlServiceType atcs = aixmFactory
                .createAirTrafficControlServiceType();
        atcs.getTimeSlice().add(atcsSliceProp);

        prop = aixmMessageFactory.createBasicMessageMemberAIXMPropertyType();
        prop.setAbstractAIXMFeature(aixmFactory
                .createAirTrafficControlService(atcs));
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void aerodromeRunwaysToAixm(Aerodrome aerodrome,
            String aerodromeId, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        runwayToAixm(aerodromeId, rwys, rwyDirRwyIdMap, validStart, validEnd,
                interpretation, sequence, correction, propList);
        ...
        for (Runway runway : aerodrome.getRunways()) {
            ...
            runwayThresholdToAixm(runway, id.getValue(), validStart, validEnd,
                    interpretation, sequence, correction, propList);
            runwayEndToAixm(runway, id.getValue(), validStart, validEnd,
                    interpretation, sequence, correction, propList);

            // create the runway element
            String rwyEId = runwayElementToAixm(runway,
                    rwyDirRwyIdMap.get(runway.getDesignator()), validStart,
                    validEnd, interpretation, sequence, correction, propList);

            // set the runway direction properties
            TextDesignatorType designator = aixmFactory
                    .createTextDesignatorType();
            ...
            ValDistanceVerticalType elevation = convertElevation(runway
                    .getElevation());

            RunwayElementPropertyType rwyEProp = aixmFactory
                    .createRunwayElementPropertyType();
            ...
            RunwayDirectionTimeSliceType rdSlice = aixmFactory
                    .createRunwayDirectionTimeSliceType();
            ...
            RunwayDirectionTimeSlicePropertyType rdSliceProp = aixmFactory
                    .createRunwayDirectionTimeSlicePropertyType();
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void runwayToAixm(String aerodromeId,
            Map<String, Runway[]> rwys, Map<String, String> rwyDirRwyIdMap,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        for (String rwyDesignator : rwys.keySet()) {
            ...
            if (sc != null) {
                SurfaceCharacteristicsType surface = aixmFactory
                        .createSurfaceCharacteristicsType();
                surface.setComposition(sc);

                surfaceProp = aixmFactory
                        .createSurfaceCharacteristicsPropertyType();
            ...
            RunwayTimeSliceType rSlice = aixmFactory
                    .createRunwayTimeSliceType();
            ...
            RunwayTimeSlicePropertyType rSliceProp = aixmFactory
                    .createRunwayTimeSlicePropertyType();
     ...
     */
    private static Map<String, Runway[]> runwayDirectionMap(Aerodrome aerodrome) {
        ...
        for (Runway runway : aerodrome.getRunways()) {
            ...
            int direction = postfix ? Integer.parseInt(designator.substring(0,
                    lastIx)) : Integer.parseInt(designator);
            ...
            if (direction < oppositeDirection) {
                if (postfix) {
                    rwyDesignator = fmt.format(direction) + lastChar + "/"
                            + fmt.format(oppositeDirection) + oppositeLastChar;
                } else {
                ...
                if (postfix) {
                    rwyDesignator = fmt.format(oppositeDirection)
                            + oppositeLastChar + "/" + fmt.format(direction)
                            + lastChar;
                } else {
                    rwyDesignator = fmt.format(oppositeDirection) + "/"
     ...
     */
    private static String runwayElementToAixm(Runway runway, String runwayId,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        CodeRunwayElementType rwyEType = aixmFactory
                .createCodeRunwayElementType();
        ...
        SurfaceCharacteristicsPropertyType surfaceProp = convertSurface(runway
                .getSurface());
        ...
        RunwayElementTimeSliceType rwyESlice = aixmFactory
                .createRunwayElementTimeSliceType();
        ...
        RunwayElementTimeSlicePropertyType rwyESliceProp = aixmFactory
                .createRunwayElementTimeSlicePropertyType();
     ...
     */
    private static ElevatedSurfacePropertyType runwayToElevatedSurface(
            Runway runway) {
        ...
        ElevatedSurfacePropertyType esProp = aixmFactory
                .createElevatedSurfacePropertyType();
     ...
     */
    private static void runwayCalcPolygon(Runway runway,
     ...
     *          supplied surface
     */
    private static SurfaceCharacteristicsPropertyType convertSurface(
            org.openaviationmap.model.SurfaceType surface) {
        // handle the surface
        SurfaceCharacteristicsPropertyType surfaceProp = null;
        ...
        if (sc != null) {
            SurfaceCharacteristicsType sfc = aixmFactory
                    .createSurfaceCharacteristicsType();
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void runwayThresholdToAixm(Runway runway, String id,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        CodeRunwayPointRoleType role = aixmFactory
                .createCodeRunwayPointRoleType();
        role.setValue("THR");

        ElevatedPointType loc = convertPointElevation(runway.getThreshold(),
                runway.getElevation());
        ElevatedPointPropertyType locProp = aixmFactory
                .createElevatedPointPropertyType();
        locProp.setElevatedPoint(loc);

        RunwayDirectionPropertyType rdp = aixmFactory
                .createRunwayDirectionPropertyType();
        rdp.setHref("#uuid." + id);

        RunwayCentrelinePointTimeSliceType thresholdSlice = aixmFactory
                .createRunwayCentrelinePointTimeSliceType();
        ...
        if (runway.getAsda() != null) {
            RunwayDeclaredDistancePropertyType asdaProp = convertDeclaredDistance(
                    runway.getAsda(), "ASDA");
        ...
        RunwayCentrelinePointTimeSlicePropertyType thrSliceProp = aixmFactory
                .createRunwayCentrelinePointTimeSlicePropertyType();
        thrSliceProp.setRunwayCentrelinePointTimeSlice(thresholdSlice);

        RunwayCentrelinePointType rct = aixmFactory
                .createRunwayCentrelinePointType();
        rct.getTimeSlice().add(thrSliceProp);

        BasicMessageMemberAIXMPropertyType prop = aixmMessageFactory
                .createBasicMessageMemberAIXMPropertyType();
        prop.setAbstractAIXMFeature(aixmFactory
                .createRunwayCentrelinePoint(rct));
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void runwayEndToAixm(Runway runway, String id,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        CodeRunwayPointRoleType role = aixmFactory
        ...
        RunwayCentrelinePointTimeSliceType endSlice = aixmFactory
                .createRunwayCentrelinePointTimeSliceType();
        initTimeSlice(endSlice, validStart, validEnd, interpretation, sequence,
                correction);
     ...
     */
    private static RunwayDeclaredDistancePropertyType convertDeclaredDistance(
            Distance distance, String type) {
        CodeDeclaredDistanceType t = aixmFactory
                .createCodeDeclaredDistanceType();
        t.setValue(type);

        ValDistanceType d = convertDistance(distance);

        RunwayDeclaredDistanceValueType dist = aixmFactory
                .createRunwayDeclaredDistanceValueType();
        dist.setDistance(d);

        RunwayDeclaredDistanceValuePropertyType distProp = aixmFactory
                .createRunwayDeclaredDistanceValuePropertyType();
        distProp.setRunwayDeclaredDistanceValue(dist);

        RunwayDeclaredDistanceType dd = aixmFactory
                .createRunwayDeclaredDistanceType();
        dd.setType(t);
        dd.getDeclaredValue().add(distProp);

        RunwayDeclaredDistancePropertyType ddProp = aixmFactory
                .createRunwayDeclaredDistancePropertyType();
     ...
     *        if in doubt, use  0
     */
    private static void initTimeSlice(AbstractAIXMTimeSliceType slice,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction) {
     ...
     */
    private static ValDistanceType convertDistance(Distance distance) {
     ...
     */
    private static DirectPositionType convertPoint(Point point) {
     ...
     */
    private static ValDistanceVerticalType convertElevation(Elevation elevation) {
        if (elevation == null) {
            return null;
        }

        ValDistanceVerticalType elev = aixmFactory
                .createValDistanceVerticalType();
     ...
     *          parameters.
     */
    private static ElevatedPointType convertPointElevation(Point point,
            Elevation elevation) {
    ...
    /**
     * Convert a VOR/TACAN into an AIXM navaid.
     *
     * @param navaid the navigation aid to convert
     * @param validStart the beginning of the validity period for the
     *        aerospace features. if null, this is unknown.
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    static void vortacToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    static void vordmeToAixm(Navaid navaid, GregorianCalendar validStart,
            GregorianCalendar validEnd, String interpretation, long sequence,
            long correction, List<BasicMessageMemberAIXMPropertyType> propList,
            List<AbstractNavaidEquipmentType> equipmentList) {
     ...
     * @param propList the converted AIXM elements will be put into this list
     */
    private static void navaidForNavaidEquipment(Navaid navaid, String type,
            List<AbstractNavaidEquipmentType> equipmentList,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {
        ...
        for (AbstractNavaidEquipmentType eq : equipmentList) {
            NavaidEquipmentPropertyType pe = aixmFactory
                    .createNavaidEquipmentPropertyType();
            ...
            NavaidComponentPropertyType p = aixmFactory
                    .createNavaidComponentPropertyType();
            p.setNavaidComponent(nt);

            slice.getNavaidEquipment().add(p);
        }

        NavaidTimeSlicePropertyType sliceProp = aixmFactory
                .createNavaidTimeSlicePropertyType();
     ...
     *        this list
     */
    public static void navaidsToAixm(List<Navaid> navaids,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {

        for (Navaid navaid : navaids) {
            navaidToAixm(navaid, validStart, validEnd, interpretation,
                    sequence, correction, propList);
        }
    }
     ...
     *        this list
     */
    public static void aerodromesToAixm(List<Aerodrome> aerodromes,
            GregorianCalendar validStart, GregorianCalendar validEnd,
            String interpretation, long sequence, long correction,
            List<BasicMessageMemberAIXMPropertyType> propList) {

        for (Aerodrome aerodrome : aerodromes) {
            aerodromeToAixm(aerodrome, validStart, validEnd, interpretation,
                    sequence, correction, propList);
        }
    }
}
